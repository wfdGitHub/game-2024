		...
		if(book.count > 0){
			if(book.rate == 1 || this.seeded.random("斩杀天书") < book.rate){
				...
				for(var i = 0;i < book.enemy.length;i++){
					if(!book.enemy[i].died && !book.enemy[i].buffs["delay_death"] && !book.enemy[i].neglect_seckill && !book.enemy[i].buffs["banish"] && (!book.enemy[i].buffs["ghost"] || book.ghost) && !book.enemy[i].isBoss){
