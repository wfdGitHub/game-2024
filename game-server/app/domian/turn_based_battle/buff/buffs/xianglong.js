...
model.prototype.buffOtps = function(attacker,info) {
	if(info.buff.maxHP){
		...
		if(target && target.checkAim()){
			var tmpValue = Math.floor(target.getTotalAtt("maxHP") * info.buff.maxHP)
	...
	if(info.buff.atk){
		...
		if(target && target.checkAim()){
			var tmpValue = Math.floor(target.getTotalAtt("atk") * info.buff.atk)
	...
	if(info.buff.armor){
		...
		if(target && target.checkAim()){
			var tmpValue = Math.floor(target.getTotalAtt("armor") * info.buff.armor)
