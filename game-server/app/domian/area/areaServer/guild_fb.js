				...
				self.redisDao.db.hget(main_name+":"+guildId,"damage_"+fbId,function(err,damage) {
					...
					if(damage < guild_fb[fbId]["hp"]){
						...
						if((damage + allDamage) >= guild_fb[fbId]["hp"]){
							//击杀
							killFlag = true
							self.sendMail(uid,"[mail_guild_boss_title]","[mail_guild_boss_text1]",currency+":"+guild_fb[fbId]["kill"])
							self.addGuildEXP(guildId,guild_fb[fbId]["exp"])
							self.addGuildGift(guildId,"Bản sao liên minh【"+(fbId+1)+"】",10,guild_fb[fbId]["gift"],oneDayTime)
							...
							self.redisDao.db.zrange(main_name+":"+guildId+":"+fbId,-3,-1,function(err,list) {
								...
								for(var i = list.length - 1;i >= 0;i--){
									rank++
									if(list[i]){
										var text = "[mail_guild_boss_rank_text1]"+rank+"[mail_guild_boss_rank_text1]"
										self.sendMail(list[i],"[mail_guild_boss_rank_title1]"+rank+"[mail_guild_boss_rank_title2]",text,guild_fb[fbId]["damage_"+rank])
									}
								}
							})
						}
