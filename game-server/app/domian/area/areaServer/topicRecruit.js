...
const default = require("../../../../config/gameCfg/default.json")
...
module.exports = function() {
	...
	this.topicRecruitOnce = function(uid,cb) {
		self.consumeItems(uid,default["topic_lotto_1"]["value"],1,function(flag,err) {
			if(!flag){
				cb(false,err)
			}else{
	...
	this.topicRecruitMultiple = function(uid,cb) {
		self.consumeItems(uid,default["topic_lotto_10"]["value"],1,function(flag,err) {
			if(!flag){
				cb(false,err)
			}else{
				self.getObj(uid,main_name,"count",function(num) {
