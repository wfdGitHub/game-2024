...
module.exports = function() {
	var self = this
	var contributions = {}		//仙盟玩家贡献列表
	var guildList = {}			//仙盟信息列表
	var guideCooling = {}		//加入冷却
	var applyList = {}			//申请列表
	var applyMap = {}			//申请映射
	var giftInfoList = {}		//仙盟红包
	//初始化
		...
		async.waterfall([
			...
			function(next) {
				self.getObj(uid,main_name,"cd",function(cd) {
					...
					if(cd > curTime){
						next("退出仙盟冷却中,"+Math.ceil((cd - curTime)/60000)+"分钟后可创建")
						return
					}
					next()
				})
			...
			function(next) {
				//扣除金额
				self.consumeItems(uid,guild_cfg["create"]["value"],1,"创建仙盟",function(flag,err) {
					if(flag)
						next()
					else
						next(err)
	...
	this.applyJoinGuild = function(uid,guildId,cb) {
		...
		if(self.getLordLv(uid) < guildList[guildId]["lv_limit"]){
			cb(false,"该仙盟需要"+guildList[guildId]["lv_limit"]+"级可加入")
			return
		}
		var lv = guildList[guildId]["lv"]
		...
		self.getObj(uid,main_name,"cd",function(cd) {
			...
			var curTime = Date.now()
			if(cd > curTime){
				cb(false,"退出仙盟冷却中,"+Math.ceil((cd - curTime)/60000)+"分钟后可申请")
				return
			}
	...
	this.joinGuild = function(uid,guildId,cb) {
		...
		self.getPlayerKeyByUid(uid,"gid",function(gid) {
			...
			self.sendMail(uid,"加入仙盟","您已成功加入【"+gname+"】")
	...
	this.leaveGuild = function(guildId,uid,cb) {
		...
		self.sendMail(uid,"退出仙盟","您已离开【"+guildList[guildId]["name"]+"】")
		...
		if(guildList[guildId]){
			...
			if(guild_lv[lv]["exp"] && guild_lv[lv+1]){
				if(exp >= guild_lv[lv]["exp"]){
					...
					self.addGuildGift(guildId,"仙盟升级红包",guild_lv[lv]["member"],guild_lv[lv]["gift_up"],oneDayTime)
	...
	this.addGuildScore = function(uid,guildId,value,reason) {
		...
		var awardList = self.addItemStr(uid,currency+":"+value,1,reason || "仙盟活动")
		return awardList
	}
	//签到
	this.signInGuild = function(uid,sign,cb) {
		...
		self.getObj(uid,main_name,"sign",function(data) {
			...
			self.consumeItems(uid,guild_sign[sign]["pc"],1,"仙盟签到",function(flag,err) {
				if(flag){
	...
	this.upGuildSkill = function(uid,career,cb) {
		...
		if(guildList[guildId]){
			...
			self.getObj(uid,main_name,"skill_"+career,function(lv) {
				...
				self.consumeItems(uid,guild_skill[lv]["pc"],1,"升级仙盟技能",function(flag,err) {
					if(flag){
	...
	this.gainGuildGift = function(uid,giftId,cb) {
		...
		self.redisDao.db.hgetall("guild:giftinfo:"+giftId,function(err,giftInfo) {
			...
			self.redisDao.db.hincrby("guild:giftinfo:"+giftId,"curNum",1,function(err,num) {
				...
				var awardList = self.addItemStr(uid,currency+":"+giftInfo["amount_"+num],1,"仙盟红包")
