...
module.exports = function() {
	...
	//挑战单人试炼
	this.challengeOneEndless = function(uid,hIds,id,seededList,index,cb) {
		...
		async.waterfall([
			...
			function(next) {
				...
				for(var i = oldIndex + 1;i <= index;i++){
					if(i % 2 == 0){
						var awardId = i / 2
						awardList = awardList.concat(self.addItemStr(uid,endless_one[id]["award_"+awardId],1,"无尽试炼"+id+"-"+awardId))
					}
				}
				var info = {
					level : level,
	...
	//挑战三人试炼
	this.challengeThreeEndless = function(argument) {
