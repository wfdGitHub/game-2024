			...
			function(next) {
				self.redisDao.db.zrevrange(["cross:grading:realRank",0,-1,"WITHSCORES"],function(err,list) {
					...
					for(var i = 0;i < list.length;i+=2){
						...
						self.sendMailByUid(uid,curSeasonId+"[mail_grading_settle_title]","[mail_grading_settle_text1][grading_lv_"+glv+"][mail_grading_settle_text2]",grading_lv[glv]["season_award"])
						if(uids.length < 6){
							areaIds.push(sid)
							uids.push(uid)
							self.sendMailByUid(uid,curSeasonId+"[mail_grading_zsd_title]","[mail_grading_zsd_text1]"+curSeasonId+"[mail_grading_zsd_text2]",grading_cfg["award_"+uids.length]["value"])
						}
					}
