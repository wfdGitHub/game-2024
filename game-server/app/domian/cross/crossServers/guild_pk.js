	...
	this.matchGuildPKRival = function() {
		...
		self.redisDao.db.hgetall(main_name+":apply",function(err,list) {
			...
			if(list){
				...
				if(arr.length > 1){
					console.log("报名仙盟大于1,开始比赛。总报名数: ",arr.length)
					var curNum = 0
					var map = {}
					var table = {}
					var tableIndex = 1
			...
			function(next) {
				...
				self.redisDao.db.hgetall(main_name+":table",function(err,table) {
					if(table){
						...
						console.log("仙盟PK 存在对阵表 开始对阵",table)
						var map = {}
						for(var tableIndex in table){
							var list = JSON.parse(table[tableIndex])
							var npc = false
			...
			function(next) {
				...
				if(atkWinNum >= 2){
					...
					for(var uid in atkMap){
						self.sendMailByUid(uid,"仙盟会武获胜奖","恭喜您的仙盟在本次仙盟会武活动中大获全胜!",guild_pk[atkGuildLv]["win"])
						self.sendMailByUid(uid,"仙盟会武参与奖","您参与本次仙盟会武活动获得了参与奖励。",guild_pk[atkGuildLv]["play"])
					}
					if(!npc){
						for(var uid in defMap){
							self.sendMailByUid(uid,"仙盟会武惜败","您的仙盟在本次会武中惜败于对手。",guild_pk[defGuildLv]["lose"])
							self.sendMailByUid(uid,"仙盟会武参与奖","您参与本次仙盟会武活动获得了参与奖励。",guild_pk[defGuildLv]["play"])
						}
					}
				}else{
					//守方赢
					for(var uid in atkMap){
						self.sendMailByUid(uid,"仙盟会武惜败","您的仙盟在本次会武中惜败于对手。",guild_pk[atkGuildLv]["lose"])
						self.sendMailByUid(uid,"仙盟会武参与奖","您参与本次仙盟会武活动获得了参与奖励。",guild_pk[atkGuildLv]["play"])
					}
					if(!npc){
						for(var uid in defMap){
							self.sendMailByUid(uid,"仙盟会武获胜奖","恭喜您的仙盟在本次仙盟会武活动中大获全胜!",guild_pk[defGuildLv]["win"])
							self.sendMailByUid(uid,"仙盟会武参与奖","您参与本次仙盟会武活动获得了参与奖励。",guild_pk[defGuildLv]["play"])
						}
					}
				}
