			...
			function(next) {
				...
				if(atkWinNum >= 2){
					...
					for(var uid in atkMap){
						self.sendMailByUid(uid,"[mail_guild_pk_win_title]","[mail_guild_pk_win_text]",guild_pk[atkGuildLv]["win"])
						self.sendMailByUid(uid,"[mail_guild_pk_play_title]","[mail_guild_pk_play_text]",guild_pk[atkGuildLv]["play"])
					}
					if(!npc){
						for(var uid in defMap){
							self.sendMailByUid(uid,"[mail_guild_pk_lose_title]","[mail_guild_pk_lose_text]",guild_pk[defGuildLv]["lose"])
							self.sendMailByUid(uid,"[mail_guild_pk_play_title]","[mail_guild_pk_play_text]",guild_pk[defGuildLv]["play"])
						}
					}
				}else{
					//守方赢
					for(var uid in atkMap){
						self.sendMailByUid(uid,"[mail_guild_pk_lose_title]","[mail_guild_pk_lose_text]",guild_pk[atkGuildLv]["lose"])
						self.sendMailByUid(uid,"[mail_guild_pk_play_title]","[mail_guild_pk_play_text]",guild_pk[atkGuildLv]["play"])
					}
					if(!npc){
						for(var uid in defMap){
							self.sendMailByUid(uid,"[mail_guild_pk_win_title]","[mail_guild_pk_win_text]",guild_pk[defGuildLv]["win"])
							self.sendMailByUid(uid,"[mail_guild_pk_play_title]","[mail_guild_pk_play_text]",guild_pk[defGuildLv]["play"])
						}
					}
				}
