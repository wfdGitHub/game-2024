...
var onUserLeave = function(session) {
	...
	if(uid){
		...
		this.cacheDao.saveCache({"messagetype":"leave",time:dt,uid:uid,accId:accId,name:session.get("name"),beginTime:beginTime,ip:session.get("ip")})
		if(serverId)
