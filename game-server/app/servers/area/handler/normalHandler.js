...
normalHandler.prototype.chooseFirstHero = function(msg, session, next) {
  ...
  self.redisDao.db.hget("player:user:"+uid+":playerData","choose",function(err,data) {
    if(!data){
      ...
      var heroInfo = self.areaManager.areaMap[areaId].gainOneHero(uid,default_cfg["choose_hero"+index]["value"],4,function(flag,heroInfo) {
        self.areaManager.areaMap[areaId].setTeamByType(uid,"normal",[heroInfo.hId],function(flag) {
          next(null,{flag:flag,heroInfo:heroInfo})
        })
      })
    }else{
      next(null,{flag:false})
