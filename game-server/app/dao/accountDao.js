...
accountDao.prototype.createRobotAccount = function(cb) {
	...
	self.createAccount({unionid : uuid.v1(),head : beginHero,robot:true},function(flag,userInfo) {
		if(flag){
			var name = boyNames[Math.floor(boyNames.length * Math.random())]
			self.playerDao.createPlayer({accId : userInfo.accId,areaId:areaId,name:name,robot:true},function(playerInfo) {
				if(playerInfo){
